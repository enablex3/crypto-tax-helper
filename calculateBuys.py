...
# We want Timestamp, Amount, To Currency, Transaction Description (e.g., "Buy CRYPTO", "USD -> CRYPTO", "USDT -> CRYPTO"), and Native Amount. We can ignore all others
...
with open(crypto_transactions_file, 'r') as csvFile:
...
for record in csv_content[1:len(csv_content)]:
    if "Buy" in record[columns["Transaction Description"]]:
    ...
    elif "USD ->" in record[columns["Transaction Description"]]:
        txid = "txid: {}".format(str(csv_content.index(record)))
        crypto_transactions[txid] = {}
        crypto_transactions[txid]["Date Acquired"] = record[columns["Timestamp"]]
        if record[columns["To Currency"]] != "":
...
with open(fiat_transactions_file, 'r') as csvFile:
    reader = csv.reader(csvFile)
    for row in reader:
        csv_content.append(row)
...
with open('buys/cryptoComBuys.csv', 'w') as csvFile:
    ...
    for transaction in crypto_transactions.values():
        writer.writerow([transaction["Date Acquired"], transaction["Property Description"].replace("-", ""), transaction["Amount (USD)"]])
